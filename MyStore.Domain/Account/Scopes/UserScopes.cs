    ...
    public static class UserScopes
    {
        ...
        public static bool RequestLoginScopeIsValid(this User user, string username)
        {
            return AssertionConcern.IsSatisfiedBy(
                ...
                AssertionConcern.AssertAreEquals(
                    DateTime.Compare(
                        user.LastAuthorizationCodeRequest.AddMinutes(-5), DateTime.Now).ToString(),
                        (-1).ToString(), 
                        "Um SMS já foi enviado. Aguarde 5 minutos requisitar um novo login"
                )
        ...
        public static bool LoginScopeIsValid(this User user,string authorizationCode, string password)
        {
            return AssertionConcern.IsSatisfiedBy(
                ...
                AssertionConcern.AssertTrue(user.Verified == true, "E-mail não verificado"),
                AssertionConcern.AssertTrue(user.Active == true, "Cadastro não ativado"),
                AssertionConcern.AssertAreEquals(user.AuthorizationCode.ToLower(), authorizationCode.ToLower(), "Código de autenti~ção inválido"),
                AssertionConcern.AssertAreEquals(user.Password, password, "Usuário ou senha inálidos"),
                AssertionConcern.AssertAreEquals(
                    DateTime.Compare(
                        user.LastAuthorizationCodeRequest.AddMinutes(5), DateTime.Now).ToString(),
                        (1).ToString(),
